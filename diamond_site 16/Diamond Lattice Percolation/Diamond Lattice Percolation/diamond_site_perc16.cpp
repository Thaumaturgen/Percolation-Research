...
// Translation to C++/CUDA by Cary Rock, August 8, 2016
...
// DON'T FORGET TO #INCLUDE GUARD THE HEADERS AND SUCH
// #ifndef 
// #define
#include <time.h>
#include <math.h>
#include <random> // as rand
// from numpy import *
// from scipy import *
#include <stdio.h>
...
// This is actually going to be a .h file containing all the above described goodies

#include "diamond_site_neighbor16.h" // as site_definition

using namespace std;

lattice = diamond_site_neighbor16.lattice_sites;
...
if (DEBUGMODE = true)  // This check needs to go in main; otherwise will error
{
	int null = lattice(0);
	int L = lattice(0):
	int NUM_RUN = 1;
...
{
	int L = 16;                      // Number of units cells on a lattice axes.
	int NUM_CPU = 1;
	float NUM_RUN = 1000 / NUM_CPU + 1;  // The number of lattices to test.
}

int S = site_definition.NUM_SITES;  // Do not edit! The number of sites per unit.
int N = (L**3)*S;         // Do not edit! Total number of sites.
int EMPTY = -N - 1;         // Do not edit!

double EPSILON = 0.00000001;         // Do not edit!


int ptr = zeros(N);                 // array of N pointers  
int order = [i for i in xrange(N)];  // occupation order
