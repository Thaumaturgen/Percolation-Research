...
using namespace std; // Don't use this, call std:: for each item that needs it

int id_to_vector(int site_id, int L);

int NUM_SITES = 2;
string NAME = "Diamond_Site";


int id_to_vector(int site_id, int L) // finish converting to array
{
	int s = site_id % NUM_SITES;
	int u = (site_id / NUM_SITES) % L;
	int v = (site_id / (NUM_SITES * L)) % L;
	int w = (site_id / (NUM_SITES * L ^ 2)) % L;
	return[s, u, v, w]; //, dtype = np.int32;
}
...
def lattice_sites(site_id, L = 16)
{
...
def lattice_displacement()
{
	/* """Vector displacements for the lattices; for each site on the unit lattice.
		...
		site_displacements.append(np.array([1., 0., 0.]))
